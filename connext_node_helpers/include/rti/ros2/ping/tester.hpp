...
#ifndef RTI__ROS2__PING__TESTER_HPP_
#define RTI__ROS2__PING__TESTER_HPP_
...
protected:
  ...
  {
    using rti::ros2::node::dds_domain_participant;
    using rti::ros2::node::datawriter_qos_profile;
    using rti::ros2::node::create_datawriter;
    using rti::ros2::node::datareader_qos_profile;
    using rti::ros2::node::create_datareader;
    using rti::ros2::node::NodeTopicKind;
    ...
    auto participant = dds_domain_participant(*this);
    ...
    // matched the writer).
    auto writer_qos = datawriter_qos_profile(*this, writer_profile);
    ...
    writer_ = create_datawriter<T>(*this, writer_topic, writer_qos,
      NodeTopicKind::Topic, false, test_options_.type_name.c_str(), &publisher_);

    auto reader_qos = datareader_qos_profile(*this, reader_profile);
    ...
    reader_ = create_datareader<T>(*this, reader_topic, reader_qos,
      NodeTopicKind::Topic, false, test_options_.type_name.c_str(), &subscriber_);
  }
  ...
  virtual void initialize_events()
  {
    ...
    reader_event_ = events_.notify_status_changed<T>(reader_,
      [this](dds::sub::DataReader<T>&) {
        on_reader_active();
      },
      StatusMask::subscription_matched() | StatusMask::data_available());
    
    writer_event_ = events_.notify_status_changed<T>(writer_,
      [this](dds::pub::DataWriter<T>&) {
        on_writer_active();
      },
    ...
    // notified.
    wd_event_ = events_.notify_user_event(
      [this](dds::core::cond::GuardCondition & condition) {
    ...
    wd_ = this->create_wall_timer(100ms,
      [this]() {
        if (is_test_ready()) {
          // Cancel watchdog timer
          wd_->cancel();
          // Trigger watchdog condition to wake up waitset
          wd_event_->guard_condition().trigger_value(true);
        }
      });
  }
  ...
  virtual uint64_t ts_now()
  {
    ...
    return participant.current_time().to_microsecs();
  }
  ...
  std::shared_ptr<rti::ros2::node::ReaderStatusEvent<T>> reader_event_;
  std::shared_ptr<rti::ros2::node::WriterStatusEvent<T>> writer_event_;
  std::shared_ptr<rti::ros2::node::UserEvent> wd_event_;
...
#endif  // RTI__ROS2__PING__TESTER_HPP_
